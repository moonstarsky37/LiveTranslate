...
from sublume.model_manager import funasr_profile, normalize_funasr_model_key

log = logging.getLogger("Sublume.FunASR")
...
class FunASREngine:
    ...
    def __init__(
        ...
        if self.family == "sensevoice":
            from sublume.asr.sensevoice import SenseVoiceEngine

            self._engine = SenseVoiceEngine(device=device, hub=hub, pad_seconds=pad_seconds)
        elif self.family == "funasr-nano":
            from sublume.asr.funasr_nano import FunASRNanoEngine
