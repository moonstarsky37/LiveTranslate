...
Extracted verbatim from SublumeApp (Phase 3 split). ASRSupervisor is a
host-object helper: it owns the ASR worker client and its bookkeeping, and
reaches back into the owning SublumeApp (``self._app``) for the state that
...
from sublume.asr.client import (
    ASRWorkerError,
    ASRWorkerExited,
    ASRWorkerTimeout,
)
