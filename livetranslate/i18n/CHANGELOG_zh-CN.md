# 更新日志

## 2026-08-08 (v2026.08.08-tw.4)
- 悬浮窗的运行/暂停按钮改为显示「按下去会做的事」: 运行中显示「暂停」、暂停中显示「开始」。先前显示的是当前状态, 在「按钮=动作」的直觉下读起来刚好相反
- 设置 → 样式新增「悬浮窗版面」三选一: 完整 (维持现状) / 精简 (按钮加大, 四个开关收进「⋯」菜单) / 单列 (只留运行暂停与设置, 其余收进菜单, 标题栏高度 62px → 32px)
- 修复按下「设置」后控制面板被压在悬浮窗底下找不到: 面板改为同样置顶并取得焦点; 面板已开但没有焦点时, 按设置改为把它叫到最前面, 不再变成隐藏 (先前按下去画面毫无反应)
- 设置 → 缓存现在会列出所有缓存目录, 不再只认识内置清单: 未知模型、中断的半套下载、下载留下的残骸都会现形, 「全部删除」也真的删得干净 (先前这些项目隐形, 删完看似清空实则还占着硬盘)

## 2026-08-08 (v2026.08.08-tw.3)
- 安装流程改用 uv: 不需要事先安装 Python, 安装脚本会获取 uv 专用的 Python 3.12, 不再搜寻 (或误用) 系统 Python
- 修复 install.bat 把虚拟环境建在 `scripts\` 下、导致 start.bat 找不到环境的问题 (影响 v2026.08.07-tw.1 起的源码安装)
- 悬浮窗监控栏新增 ASR 状态: 加载中 / 无法使用一目了然, 启动与切换引擎期间不再像卡死
- ASR 模型加载后先跑一次预热推理: 加载完成后的第一句不再多等 5-6 秒
- 暂停时把讲到一半的句子送出识别再清空缓冲: 恢复后不再冒出暂停前的陈旧语音, 那半句也不会凭空消失 (与关闭程序时的行为一致)
- 设置 → 识别新增「最低语音密度」滑块 (设 0 关闭): 停顿多的来源 (剪过的视频、讲话慢) 不再被整段当噪音丢弃
- 设置 → 缓存新增 HuggingFace token (选填): 解除匿名下载限流, 内容不会写进日志
- 修复下载时「unauthenticated requests」英文警告重复出现 (huggingface_hub 1.27 会在加载时覆盖日志等级, 改用环境变量压制)

## 2026-08-08 (v2026.08.08-tw.1)
- 出厂默认清理: 移除继承自上游的默认 API 密钥 (config.yaml 改为本地 llama-server + translategemma 建议值, 密钥用惯例值 sk-local), 首启向导的 API 预填同步改自出厂默认
- 首启 API 配置提示改为中性文案: 任何 OpenAI 兼容 API 皆可, 不再推荐特定服务
- 防呆: API 密钥为空时不再于启动时崩溃 (client 端自动代入无效占位密钥, 云端服务仍会返回正常的认证错误)

## 2026-08-07 (v2026.08.07-tw.1, 本 fork 首次发版)
- 本版起为 moonstarsky37/LiveTranslate fork: 模型下载全面改走 HuggingFace (曾以 ModelScope 下载的旧缓存仍可读)
- 首启向导重设计: 移除 15 秒自动倒计时, 下载只在明确点击按钮后开始; 设置在点击当下即写入, 下载中断后重启直接续走缺模型下载流程, 不再重回向导循环
- 繁体中文 (zh-TW) 升为第一级语言: 新增繁中界面与繁中主文档, zh-TW/zh-HK 系统默认繁中; 旧设置 "zh" 自动迁移为 zh-TW
- 修复 VAD: 短于最短语音长度的尾句在持续静音后改为强制送出, 暂停前的最后一句不再消失
- 修复 requirements.txt 漏列 pysbd 导致全新安装在增量识别时崩溃的问题
- 内部重构 (Phase 1-3): 代码 package 化 (livetranslate/), 设置统一入口 SettingsStore, main / control_panel / overlay / supervisor 巨石文件全数拆分, 行为以 AST 比对与特性测试钉住
- 建立 CI 质量关卡 (ruff + mypy + pytest) 与 branch protection: CI 不绿进不了 main
- (tw.2) 下载体验改善: 停用 HuggingFace Xet CDN 避开间歇性 500/CAS 中断; 下载失败改显示精简错误与重试提示, 完整 traceback 只进文件日志; 压掉 symlink 与匿名限流的英文警告墙
- (tw.2) 首启向导与模型下载对话框补回关闭按钮 (X); 中途关闭安全, 下次启动由缺模型流程续传

## 2026-07-11
- 修复 Fun-ASR-Nano 首次加载时 Qwen3-0.6B 权重下载被 180 秒超时打断的问题 (#32): 权重改为在模型下载阶段提前获取, ASR worker 启动不再等待大文件下载
- 修复 ModelScope 1.38+ 缓存位置变化导致模型漏检的问题 (#32, #33): 兼容各版本 SDK 的全部缓存目录布局, 升级 SDK 后不再重复下载已有模型
- Qwen3-0.6B 权重支持从 ModelScope 下载, ModelScope 源用户不再被迫走 HuggingFace (#33)

## 2026-06-20
- 新增「远程 Whisper」ASR 引擎: 把语音识别外包到带 GPU 的另一台机器 (附 `asr_server.py` 服务端), 无 GPU 的机器也能实时识别
- 新增「WebID 身份核验」翻译提示词预设: 针对视频核验场景调优用词
- ASR 改为子进程隔离运行: worker 崩溃/超时自动重启, 内存增长到阈值自动回收, 识别故障不再拖垮界面
- 字幕窗口支持鼠标穿透 (#28): 字幕设置里新增开关, 托盘菜单可「字幕鼠标穿透」快速切换, 开启后点击直达背后窗口 (此时中键拖动失效, 先关穿透再移动)

## 2026-05-10
- 新增「导出到文件」: 支持原文 / 译文 / 原文+译文 三种格式, 悬浮窗右键菜单与托盘菜单均可触发
- 新增「转录持久化」(默认开启): 每次会话自动在 `transcripts/` 下创建 3 份文件 (原文 / 译文 / 全部), 每段识别结果实时追加写入, 不再受悬浮窗 50 条上限丢失早期文本
- 设置面板「缓存」标签新增「转录持久化」分组: 开关 + 打开转录文件夹按钮
- 新增内存占用上限保护: RSS 超过 4096MB 时弹托盘通知提示重启 (ASR 后端存在 native 侧工作区/缓存, Python GC 与 `torch.cuda.empty_cache` 不一定能回收)
- 日志新增 `MEM[asr#/tick]` 行: 每次真实 ASR 调用后输出 RSS / GPU(alloc/reserved) / 音频时长 / overlay 消息数 / VAD buffer 长度, 便于排查内存问题

## 2026-04-20
- 移除 Qwen3-ASR 引擎 (ONNX + GGUF 方案兼容性较差, 相关模型文件和 llama.cpp 运行时依赖一并清理)
- 模型配置新增「高级参数」: `temperature` / `top_p` / `max_tokens` / `frequency_penalty` / `presence_penalty` / `seed`, 每项独立「覆盖」开关, 未勾选时使用服务端默认值
- 模型配置新增 `extra_body` (JSON): 供应商专有参数透传, 如 `thinking_budget`、`reasoning_effort` 等, 保存时自动校验 JSON 格式
- 修复 Anime-Whisper 模型未缓存时下载对话框静默无动作的问题
- 修复设置面板「更新日志」Tab 空白 (正则匹配 H3 但文件用 H2, 3 月加入功能起就失效)

## 2026-04-18
- 新增 ASR 引擎: Anime-Whisper (litagin/anime-whisper), 日语动画/Galgame 特化, 擅长识别喘息/叹息等非语言发声
- 修复 HF 缓存检测误判: 下载中断留下的空目录不再被认为"已缓存"

## 2026-03-31
- 管道线程拆分: 原 capture+VAD+ASR 单线程改为 capture 与 ASR 两条独立线程, 长句 ASR 不再阻塞 RMS/VAD 条实时更新
- ASR 调度新增有界队列 (16 段), 队列满时丢弃最旧的 interim 段, 避免堆积导致延迟

## 2026-03-26
- 默认翻译提示词优化: 新增 ASR 纠错规则 (识别错别字/同音字时尽量按上下文修正)、流畅性规则 (避免逐字直译)

## 2026-03-25
- 样式标签新增「重置窗口位置」按钮: 字幕窗口回到 (100,100), 主悬浮窗回到屏幕右下角
- 字幕窗口默认位置从屏幕底部居中改为 (100,100), 最小高度调整为 200px
- 窗口位置恢复时校验是否在可见屏幕范围内 (使用 availableGeometry 排除任务栏), 高度变化后 clamp 到屏幕边界

## 2026-03-24
- 字幕窗口: 超长文本自动换行显示(不再分段), 背景高度平滑动画, 文字渲染 Pixmap 缓存
- 主悬浮窗/字幕窗口: 位置和大小自动记忆, 重启后恢复
- 主悬浮窗: 精简模式切换动画
- 设置: 移除配置文件 key 白名单限制

## 2026-03-23
- 品牌更名 LiveTrans → LiveTranslate
- 模型配置新增: 流式传输、结构化输出、上下文数、禁用思考(默认开启)
- 翻译结果流式逐字显示
- 提示词优化: 禁止多候选翻译, 编辑即时生效
- 模型重复输出检测与提示
- ASR 引擎标注[准确]/[快速]
- 设置面板新增更新日志 Tab
